...
const Hero = () => {
  ...
  const StadiumField = () => (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div 
        className="absolute inset-0 bg-gradient-to-br from-blue-800/20 via-blue-700/15 to-blue-900/20"
        style={{
      ...
      >
        {/* Center line */}
        <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-blue-800 transform -translate-y-1/2" />
        
        {/* Center circle */}
        <div className="absolute top-1/2 left-1/2 w-32 h-32 border-2 border-blue-800 rounded-full transform -translate-x-1/2 -translate-y-1/2" />
        <div className="absolute top-1/2 left-1/2 w-1 h-1 bg-blue-800 rounded-full transform -translate-x-1/2 -translate-y-1/2" />
        
        {/* Goal areas - simplified */}
        <div className="absolute top-1/3 left-8 w-16 h-24 border-2 border-blue-800" />
        <div className="absolute top-1/3 right-8 w-16 h-24 border-2 border-blue-800" />
      </div>
      ...
      <div 
        className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-blue-900/60 to-transparent"
        style={{
  ...
  const Football3D = () => (
    ...
    >
      <div className="relative">
        <div className="w-32 h-32 rounded-full bg-gradient-to-br from-white via-blue-50 to-blue-100 shadow-xl border-2 border-blue-300 relative overflow-hidden group-hover:scale-105 transition-all duration-300">
          {/* Realistic football pattern */}
          <div className="absolute inset-0 rounded-full">
            {/* Central pentagon */}
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-blue-800 rounded-lg rotate-45 opacity-70" />
            
            {/* Surrounding hexagons */}
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="absolute w-6 h-6 border-2 border-blue-800 rounded-lg opacity-60"
                style={{
          ...
          {/* Subtle glow effect */}
          <div className="absolute inset-0 rounded-full bg-gradient-to-r from-blue-400/10 to-blue-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        ...
        {/* Subtle 3D shadow */}
        <div className="absolute inset-0 bg-blue-800/20 rounded-full transform translate-y-2 blur-md scale-90 opacity-50" />
      </div>
    </div>
  );
  ...
  const FloatingImage = () => (
    <div 
      ...
      className="absolute top-1/4 right-1/4 w-24 h-24 rounded-full overflow-hidden border-4 border-blue-400/50 shadow-2xl shadow-blue-500/30 transition-all duration-300"
      style={{
  ...
  const GoalPost3D = ({ side }) => (
    ...
    >
      <div className="hover:scale-105 transition-all duration-300">
        ...
        <div className="relative">
          {/* Vertical posts */}
          <div className="w-2 h-24 bg-gradient-to-b from-blue-200 to-blue-400 rounded-full shadow-md" />
          <div className="w-2 h-24 bg-gradient-to-b from-blue-200 to-blue-400 rounded-full shadow-md absolute left-16" />
          
          {/* Horizontal crossbar */}
          <div className="w-16 h-2 bg-gradient-to-r from-blue-200 to-blue-400 rounded-full shadow-md absolute top-0 left-0" />
          
          {/* Simple net indication */}
          <div className="absolute top-2 left-2 w-12 h-20 opacity-40">
            <div className="w-full h-full border border-blue-400 rounded-sm" style={{
              backgroundImage: `
  ...
  const ScoreBoard = () => (
    <div 
      className="absolute top-4 right-4 bg-blue-900/70 backdrop-blur-sm border border-blue-400/20 rounded-lg p-3 z-20"
      style={{
    ...
    >
      <div className="text-center">
        <div className="text-blue-400 text-xs font-semibold mb-1">United FC Kodagu</div>
        <div className="text-blue-900 text-xl font-bold">∞</div>
        <div className="text-blue-600 text-xs">CHAMPIONS</div>
      </div>
    </div>
  );
  ...
  return (
    <section 
      ref={heroRef}
      id="home" 
      className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-white via-blue-50/20 to-blue-50/15 transition-colors duration-300"
    >
      ...
      <div className="relative z-10 text-center text-blue-900 max-w-6xl mx-auto px-4 flex flex-col items-center justify-center">
        <h1 className="text-6xl md:text-8xl font-bold mb-8 leading-tight">
          <span className="bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 bg-clip-text text-transparent animate-gradient-x relative sports-title">
            United FC Kodagu
          </span>
        </h1>
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-center mb-12">
          <button 
            ...
            className="group relative bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white px-10 py-4 rounded-full font-semibold text-lg transition-all transform hover:scale-105 flex items-center gap-3 shadow-xl shadow-blue-500/20 hover:shadow-blue-400/30"
          >
          ...
          <Magnet padding={60} disabled={false} magnetStrength={40}>
            <button className="group relative border-2 border-blue-400/50 text-blue-500 hover:bg-blue-400/10 hover:border-blue-400 px-10 py-4 rounded-full font-semibold text-lg transition-all flex items-center gap-3 backdrop-blur-sm">

...
const LatestVideos = () => {
  ...
  return (
    <div className='h-fit px-[5vw] py-12 flex flex-col '>
          ...
          videos.map((item, idx) => (
            ...
            >
              <div className='h-full w-full relative overflow-hidden'>
                ...
                />
                {!playing || playing !== idx ? (
                  <div className='absolute inset-0 bg-black/40 flex items-center justify-center text-white text-2xl font-bold opacity-80'>
                    ▶
                  </div>
                ) : null}
              </div>
      ...
      {/* Modal Video Player */}
      {modalVideo && (
        <div className='fixed inset-0 z-[2000] bg-black/80 flex items-center justify-center' onClick={() => setModalVideo(null)}>
          <video src={modalVideo} controls autoPlay className='w-[80vw] max-h-[80vh] rounded-lg shadow-2xl bg-black' />
        </div>
      )}
    </div>
  );
